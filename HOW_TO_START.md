...
4. Если вы задеплоили новую модель на новый URL, вы можете [вернуться к гугл-форме](https://forms.gle/9yxRsMXGRspQTaTi6) и отредактировать URL в ней. Лидерборд остается [на том же месте](https://hat-leaderboard.herokuapp.com) :)
...
Обратим внимание на класс `RemotePlayer`. Этот класс используется для коммуникации с сервисом, поднимаем вами на удаленной (или локальной) машине. Смотря на него, видно, какие методы (эндпоинты) дергаются у сервиса. Сам запускаемый сервис описан в папке flask_app.
