...
5. Если вы задеплоили новую модель на новый URL, вы можете [вернуться к гугл-форме](https://forms.gle/9yxRsMXGRspQTaTi6) и отредактировать URL в ней. Лидерборд остается [на том же месте](https://hat-leaderboard.herokuapp.com) :)
...
Начнем с примера проведения игры. Для этого откроем ноутбук `GameRun_Demo.ipynb` и выполним его целиком. Мы будем использовать 20 news groups - их можно загрузить с помощью модуля [`sklearn.datasets`](http://scikit-learn.org/stable/modules/generated/sklearn.datasets.fetch_20newsgroups.html). Они также доступны на [kaggle](https://www.kaggle.com/crawford/20-newsgroups).
...
Также проводить локальные игры можно с помощью скрипта [run_game.py](run_game.py).
