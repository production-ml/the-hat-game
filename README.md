...
Обратите внимание на то, что есть две стратегии заворачивания модели в докер-образ: запаковать файл с моделью в докер-образ (докер-образ будет весить много мегабайт), скачивать модель при каждом старте сервиса (сервис будет долго запускаться и перезапускаться). Для временного хранения файлов удобно использовать [transfer.sh](transfer.sh) (примеры на по ссылке).
